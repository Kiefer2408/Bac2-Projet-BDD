...
class SQL:
    ...
    def pConvert(self,argument,RName):
        sqlStr = f"(SELECT DISTINCT {self.getAlias()} from {RName} {self.getAlias()})"
        return sqlStr
    ...
    def rConvert(self,oldName,newName,dbFileName,RName):
        columns_name = ",".join(self.getDbKeys(dbFileName,RName)).replace(oldName, f"{oldName} AS {newName}")
    ...
    def checkSameAtribute(self,RName1,RName2):
        keys1=self.getDbKeys(self.dbFileName,RName1)
        keys2=self.getDbKeys(self.dbFileName,RName2)
